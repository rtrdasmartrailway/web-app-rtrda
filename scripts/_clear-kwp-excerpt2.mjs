...
// Backup (already have kwp_excerpt_backup_v1_th from earlier)
await p.siteMeta.upsert({
  where: { key: "kwp_excerpt_backup_v2_th" },
  update: { value: r.excerpt || "" },
  create: { key: "kwp_excerpt_backup_v2_th", value: r.excerpt || "" },
});

console.log("Old excerpt:", r.excerpt?.slice(0, 100));
