...
const links = [...new Set([...r.contentHtml.matchAll(/<a[^>]+href="(\/sdc_download\/\d+|\/3d-flip-book\/[^"]+)"/g)].map(m => m[1]))];
...
for (const href of links) {
  ...
  try {
    ...
    if (code < 400) ok++; else { fail++; failures.push(`${code}  ${href}`); }
  } catch { fail++; failures.push(`ERR  ${href}`); }
}
