...
const v0 = await p.siteMeta.findUnique({
  where: { key: "klang415_content_backup_v1_th" },
});
if (!v0) {
...
const json = JSON.parse(
  (await import("node:fs/promises")).readFileSync("src/data/wp-content.json", "utf8"),
);
const orig = json.records.find((r) => r.path === "/คลังความรู้");
if (!orig) {
  console.error("not in JSON");
  process.exit(1);
}
