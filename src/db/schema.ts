...
// ─── Auth (Better Auth) ───────────────────────────────────────────────────────
//
// These four tables back Better Auth (Microsoft Entra ID sign-in). Property keys
// are camelCase to match the field names Better Auth's Drizzle adapter expects;
// DB column names are snake_case. IDs are text (Better Auth issues string IDs) —
// kept deliberately separate from the serial IDs of the content tables.
//
// `role` is our custom RBAC field (see src/lib/permissions.ts). It is admin-managed
// only — Better Auth is configured with `input: false` so it can't be self-assigned.
